...
type Props = {
  ...
  selectedThread: AdminContactThread | null
  status: AdminSupportTicketStatus
  tickets: AdminContactTicket[]
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Public website messages with outbound email replies and cleanup controls.
          </p>
        </div>
      ...
      <div className="grid xl:grid-cols-[minmax(0,1fr)_440px]">
        <div className="divide-y divide-slate-100">
          {tickets.map((ticket) => (
            <ContactRow
              ...
              isUpdating={isLoading || updatingTicketId === ticket.id}
              onDelete={onDelete}
        ...
        <ContactThread
          isUpdating={isLoading || Boolean(selectedTicketId && updatingTicketId === selectedTicketId)}
          onReply={onReply}
...
function ContactRow({
  ...
  return (
    <article className={`p-5 ${isSelected ? 'bg-pink-50/70' : ''}`}>
      ...
      <p className="mt-1 text-sm text-slate-500">
        {ticket.contact_name || 'Public contact'} · {ticket.contact_email || 'No email'} ·{' '}
        {formatDate(ticket.last_message_at || ticket.created_at)}
      </p>
      <div className="mt-4 flex flex-wrap gap-2">
        <ActionButton disabled={isUpdating} label="Open thread" onClick={() => onOpenTicket(ticket.id)} primary />
        <ActionButton disabled={isUpdating} label="Pending user" onClick={() => onUpdateStatus(ticket.id, 'pending_user')} />
        <ActionButton disabled={isUpdating} label="Pending admin" onClick={() => onUpdateStatus(ticket.id, 'pending_admin')} />
        <ActionButton disabled={isUpdating} label="Resolve" onClick={() => onUpdateStatus(ticket.id, 'resolved')} />
        <ActionButton disabled={isUpdating} label="Delete" onClick={() => onDelete(ticket.id)} danger />
      </div>
    </article>
  )
}
...
function ContactThread({
  ...
  const ticket = selectedThread?.ticket
  ...
  if (!ticket) {
    return (
      <aside className="border-t border-slate-200 bg-slate-50 p-5 xl:border-l xl:border-t-0">
        <p className="text-sm font-black text-slate-700">No contact open</p>
        <p className="mt-2 text-sm leading-6 text-slate-500">
          Open a public contact message to read the thread and reply by email.
        </p>
      </aside>
    )
  }
  ...
  return (
    <aside className="border-t border-slate-200 bg-slate-50 p-5 xl:border-l xl:border-t-0">
      ...
      <p className="mt-1 text-sm font-bold text-slate-500">{ticket.contact_email}</p>
        ...
        {selectedThread.messages.map((message) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">
                {message.sender_role === 'admin' ? 'Admin' : 'Public contact'}
              </p>
      ...
      >
        <label className="block">
          ...
          <textarea
            ...
            placeholder="Reply to this contact email..."
          />
        </label>
        <button
          type="submit"
        ...
        >
          {isUpdating ? 'Sending...' : 'Send email reply'}
        </button>
      </form>
    </aside>
  )
...
function StatusBadge({ status }: { status: AdminContactTicket['status'] }) {
  const styles: Record<AdminContactTicket['status'], string> = {
