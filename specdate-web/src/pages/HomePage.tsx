  ...
  return (
    <div className="min-h-screen bg-gray-900 bg-[url('/bg.png')] bg-cover bg-center bg-no-repeat bg-fixed text-white">
      <Hero minHeight="min-h-[85vh]">
        <div className="absolute top-6 right-4 left-4 z-20 sm:right-6">
              ...
              {NAV_LINKS.map(({ label, href }) =>
                href.startsWith('/') ? (
                  <Link
                    key={href}
                    to={href}
                    className="text-base font-extrabold tracking-wide text-pink-500 drop-shadow-lg transition hover:text-pink-400 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 focus:ring-offset-gray-900 rounded"
                  >
                    {label}
                  </Link>
                ) : (
                  ...
                  <li key={`mobile-${href}`}>
                    {href.startsWith('/') ? (
                      <Link
                        to={href}
                        onClick={closeMenu}
                        className="block rounded-lg px-3 py-2 text-sm font-bold tracking-wide text-pink-300 transition hover:bg-pink-500/15 hover:text-pink-200 focus:outline-none focus:ring-2 focus:ring-pink-500"
                      >
                        {label}
                      </Link>
                    ) : (
